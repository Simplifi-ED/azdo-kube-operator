...
- Add `spec.env` to the `AzureDevOps` CRD to allow injecting additional environment variables into the agent Pod (including support for `AZP_CLIENTID` / `AZP_CLIENTSECRET` / `AZP_TENANTID` via Secrets).
